...
// Get chat history (public - for session owner to restore chat on refresh)
router.get("/history/:sessionId", async (req: Request, res: Response) => {
  const sid = req.params.sessionId as string;
  if (!sid || !sid.startsWith("sess_")) {
    res.json({code: 400, message: "无效的会话ID", data: null} as ApiResponse);
    return;
  }
  try {
    const db = getPool();
...
router.post("/send", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Update session message count with actual count
    await db.execute(
      "UPDATE chat_sessions SET message_count = (SELECT COUNT(*) FROM chat_messages WHERE chat_messages.session_id = ?), updated_at = CURRENT_TIMESTAMP WHERE session_id = ?",
      [sid, sid],
    );
...
router.get("/sessions", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [sessions] = await db.execute(
      "SELECT cs.*, (SELECT COUNT(*) FROM chat_messages cm WHERE cm.session_id = cs.session_id) as message_count FROM chat_sessions cs ORDER BY cs.updated_at DESC LIMIT ? OFFSET ?",
      [ps, offset],
    );
